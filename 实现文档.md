...
| 特性 | 常规模式 (本地调试) | Xvfb 模式 (生产/服务器) |
|------|---------------------|------------------------|
| 显示环境 | 使用系统默认 DISPLAY（你的显示器） | 手动启动 Xvfb 虚拟帧缓冲 `:99` |
| 浏览器窗口 | **可见** — Chrome 窗口在桌面上弹出，能观察自动操作 | **不可见** — Chrome 运行在内存中的虚拟屏幕上 |
| 适用场景 | 本地调试，方便观察脚本执行到哪一步 | 服务器/自动化环境，无需物理显示器 |
| `headless` 参数 | `False` | `False`（**不是** `--headless=new`） |
| DISPLAY 变量 | 系统默认（如 `:0`） | `DISPLAY=:99`（Xvfb 提供） |
| 资源清理 | 脚本结束，浏览器关闭即可 | 需要 `xvfb.terminate()` 杀死虚拟显示进程 |
| Cloudflare | ~2s 通过 | ~2s 通过 |
| Stripe 加载 | ~20s | ~22s |
| hCaptcha | 自动点击通过 | 自动点击通过 |

### ⚠️ `--headless=new` 不可用

| 指标 | Xvfb 模式 | `--headless=new` |
|------|-----------|-------------------|
| Cloudflare | 2s ✅ | 36s ❌ |
| Stripe 加载 | 22s ✅ | 超时失败 ❌ |
| UA 标识 | `Chrome/145` | `HeadlessChrome/145` (被检测) |
| WebGL 指纹 | SwiftShader 正常 | GPU 相关功能受限 |

**结论**: 无论本地还是服务器，都使用 `headless=False`。服务器上通过 Xvfb 提供虚拟显示即可。

### 模式 1: 常规模式 (本地调试)

```python
# 直接在桌面环境运行，Chrome 窗口可见
from browser_payment import BrowserPayment

bp = BrowserPayment(proxy="http://proxy:port", headless=False, slow_mo=80)
result = bp.run_full_flow(...)  # Chrome 窗口弹出，可以观察操作
```

### 模式 2: Xvfb 模式 (服务器/无显示器)
...
try:
    from browser_payment import BrowserPayment
    bp = BrowserPayment(proxy="http://proxy:port", headless=False, slow_mo=80)
    result = bp.run_full_flow(...)  # Chrome 运行在虚拟屏幕，无可见窗口
finally:
    xvfb.terminate()  # 记得清理 Xvfb 进程
```
...
# 开发者模式 — 显示日志、Stripe 响应、高级设置
DISPLAY=:99 streamlit run ui.py --server.port 8503 -- --dev
```
